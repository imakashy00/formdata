...
async def test_auth_login_redirect(client: AsyncClient):
    ...
    with patch(
        "app.services.oauth.oauth.google.authorize_redirect", new_callable=AsyncMock
    ) as mock_auth:
        from fastapi.responses import RedirectResponse

        mock_auth.return_value = RedirectResponse(
            url="https://accounts.google.com/o/oauth2/auth", status_code=303
        )
...
async def test_auth_logout(client: AsyncClient, auth_cookies: dict, sample_user: User):
    ...
    response = await client.post(
        "/logout", cookies=auth_cookies, follow_redirects=False
    )
